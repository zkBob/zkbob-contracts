...
contract ZkBobSequencer is MemoUtils,Parameters {
...
function bytesToHexString(bytes memory data) public pure returns (string memory) {
    ...
    function commit() external {
        ...
        // bytes memory memo =  new bytes(0);//_memo_message();
        bytes memory memo =  _memo_sequencer_data();
        console2.log("memo retrieved", bytesToHexString(memo));
        // console2.log("memo", memo);
        uint256[8] calldata transferProof = _transfer_proof();
        require(
            pendingNullifiers[_transfer_nullifier()] == 0,
            "ZkBobSequencer: nullifier is already pending"
        );
        ...
        require(
            _pool.nullifiers(nullifier) == 0,
            "ZkBobSequencer: nullifier is spent"
        );
        require(
            uint96(index) <= _pool.pool_index(),
            "ZkBobSequencer: index is too high"
        );
        require(
            _pool.transfer_verifier().verifyProof(
                _transfer_pub(),
                transferProof
            ),
        ...
        require(
            MemoUtils.parseMessagePrefix(memo) == MESSAGE_PREFIX_COMMON_V1,
            "ZkBobPool: bad message prefix"
        );
    ...
    function prove(
        CommitData calldata commitData,
        ProveData calldata proveData
    ) external {
        ...
        (address proxy, uint256 proxy_fee, uint256 prover_fee) = MemoUtils
            .parseFees(commitData.memo);
    ...
    function skip(CommitData calldata commitData) external {
        ...
        require(
            op.timestamp + EXPIRATION_TIME < block.timestamp,
            "ZkBobSequencer: not expired yet"
        );
    ...
    //     r[3] =
    //         commitData.transfer_delta +
    //         (_pool.pool_id() << (TRANSFER_DELTA_SIZE * 8));
    ...
    //     CommitData calldata commitData,
    //     ProveData calldata proveData
    // ) internal view returns (uint256[3] memory r) {
    //     r[0] = _pool.roots(_pool.pool_index());
    //     r[1] = proveData.root_after;
    //     r[2] = commitData.out_commit;
    // }

    function preparePoolTx(
        CommitData calldata commitData,
        ProveData calldata proveData
