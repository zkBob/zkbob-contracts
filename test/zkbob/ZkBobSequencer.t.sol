...
abstract contract AbstractZkBobPoolSequencerTest is AbstractForkTest {
    ...
    function testCommitProveDeposit() external {
        ...
        (bytes memory commitData, bytes memory proveData) = _encodeDeposit(
            amount,
            proxyFee,
            proverFee,
            user1
        ...
        (bool success, bytes memory result) = address(sequencer).call(
            abi.encodePacked(ZkBobSequencer.commit.selector, commitData)
        );
        assertTrue(success);

        (success, result) = address(sequencer).call(
    ...
    function testCanSkipExpiredOperation() external {
        ...
        uint64 proverFee = uint64(1);
        (
            bytes memory firstCommitData,
            bytes memory firstProveData
        ) = _encodeDeposit(amount, proxyFee, proverFee, user1);
        (
            bytes memory secondCommitData,
            bytes memory secondProveData
        ) = _encodeDeposit(amount, proxyFee, proverFee, user2);
        ...
        vm.prank(user1);
        (bool success, ) = address(sequencer).call(
            abi.encodePacked(ZkBobSequencer.commit.selector, firstCommitData)
        );
        assertTrue(success);

        vm.prank(user2);
        (success, ) = address(sequencer).call(
            abi.encodePacked(ZkBobSequencer.commit.selector, secondCommitData)
        );
        assertTrue(success);
        ...
        vm.prank(user1);
        (success, ) = address(sequencer).call(
            abi.encodePacked(ZkBobSequencer.prove.selector, firstProveData)
        );
        assertFalse(success);

        vm.prank(user2);
        (success, ) = address(sequencer).call(
            abi.encodePacked(ZkBobSequencer.prove.selector, secondProveData)
        );
        assertTrue(success);
    }
